...
void indcpa_keypair(unsigned char *pk, unsigned char *sk) {
    polyvec skpv;
    polyvec_half skpv_prime;
    // poly pkp;
    poly_double pkp;
    int32_t *pkp_tmp = (int32_t *)pkp.coeffs;
    ...
    // i = 0
    matacc_cache32((poly *)&pkp, pkp_tmp, &skpv, &skpv_prime, 0, publicseed, 0);

    poly_invntt((poly *)&pkp);
    poly_addnoise_eta1((poly *)&pkp, noiseseed, nonce++);
    poly_ntt((poly *)&pkp);
    poly_tobytes(pk, (poly *)&pkp);

    for (i = 1; i < KYBER_K; i++) {
        matacc_opt32((poly *)&pkp, pkp_tmp, &skpv, &skpv_prime, i, publicseed, 0);
        poly_invntt((poly *)&pkp);

        poly_addnoise_eta1((poly *)&pkp, noiseseed, nonce++);
        poly_ntt((poly *)&pkp);
        poly_tobytes(pk + i * KYBER_POLYBYTES, (poly *)&pkp);
    }
...
void indcpa_enc(unsigned char *c,
    ...
    poly_basemul_opt_16_32(r_tmp, &sp.vec[0], pkp, &sp_prime.vec[0]);
    for (i = 1; i < KYBER_K - 1; i++)
    {
        poly_frombytes(pkp, pk + i * KYBER_POLYBYTES);
        poly_basemul_acc_opt_32_32(r_tmp, &sp.vec[i], pkp, &sp_prime.vec[i]);
    }
    poly_frombytes(pkp, pk + i * KYBER_POLYBYTES);
    poly_basemul_acc_opt_32_16(v, &sp.vec[i], pkp, &sp_prime.vec[i], r_tmp);
