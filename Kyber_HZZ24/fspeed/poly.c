 ...
 * Name:        poly_compress
 *
 * Description: Serialization of a polynomial and subsequent compression of a
 *polynomial;
 *
 * Arguments:   - unsigned char *r: pointer to output byte array (of length
...
void poly_compress(unsigned char *r, poly *a)
{
    ...
    for (i = 0; i < KYBER_N; i += 8) {
        for (j = 0; j < 8; j++)
            t[j] =
                ((((uint32_t)a->coeffs[i + j] << 3) + KYBER_Q / 2) / KYBER_Q) &
                7;
    ...
    for (i = 0; i < KYBER_N; i += 8) {
        for (j = 0; j < 8; j++)
            t[j] =
                ((((uint32_t)a->coeffs[i + j] << 4) + KYBER_Q / 2) / KYBER_Q) &
                15;
    ...
    for (i = 0; i < KYBER_N; i += 8) {
        for (j = 0; j < 8; j++)
            t[j] =
                ((((uint32_t)a->coeffs[i + j] << 5) + KYBER_Q / 2) / KYBER_Q) &
                31;
 ...
 *
 * Description: Serialization and subsequent compression of a polynomial of a
 *polyvec, writes to a byte string representation of the whole polyvec. Used to
 *compress a polyvec one poly at a time in a loop.
 *
 * Arguments:   - unsigned char *r:  pointer to output byte string
 *representation of a polyvec (of length KYBER_POLYVECCOMPRESSEDBYTES)
 *              - const poly *a:     pointer to input polynomial
 *              - int i:             index of to be serialized polynomial in
 *serialized polyec
 **************************************************/
void poly_packcompress(unsigned char *r, poly *a, int i)
{
    ...
    for (j = 0; j < KYBER_N / 8; j++) {
        for (k = 0; k < 8; k++)
            t[k] = ((((uint32_t)a->coeffs[8 * j + k] << 11) + KYBER_Q / 2) /
                    KYBER_Q) &
                   0x7ff;
    ...
    for (j = 0; j < KYBER_N / 4; j++) {
        for (k = 0; k < 4; k++)
            t[k] = ((((uint32_t)a->coeffs[4 * j + k] << 10) + KYBER_Q / 2) /
                    KYBER_Q) &
                   0x3ff;
 ...
 *
 * Description: Serializes and consequently compares polynomial to a serialized
 *polynomial
 *
 * Arguments:   - const unsigned char *r:    pointer to serialized polynomial to
 *compare with
 *              - poly *a:                   pointer to input polynomial to
 *serialize and compare Returns:                                  boolean
 *indicating whether the polynomials are equal
 **************************************************/
 ...
 *
 * Description: Serializes and consequently compares poly of polyvec to a
 *serialized polyvec Should be called in a loop over all poly's of a polyvec.
 *
 * Arguments:   - const unsigned char *r:    pointer to serialized polyvec to
 *compare with
 *              - poly *a:                   pointer to input polynomial of
 *polyvec to serialize and compare
 *              - int i:                     index of poly in polyvec to compare
 *with Returns:                                  boolean indicating whether the
 *polyvecs are equal
 **************************************************/
int cmp_poly_packcompress(const unsigned char *r, poly *a, int i)
{
    ...
    for (j = 0; j < KYBER_N / 4; j++) {
        ...
        rc |= r[320 * i + 5 * j + 2] ^
              (((t[1] >> 6) | ((t[2] & 0x0f) << 4)) & 0xff);
        rc |= r[320 * i + 5 * j + 3] ^
              (((t[2] >> 4) | ((t[3] & 0x03) << 6)) & 0xff);
...
void poly_frombytes(poly *r, const unsigned char *a)
{
    ...
    for (i = 0; i < KYBER_N / 2; i++) {
        ...
        r->coeffs[2 * i + 1] =
            a[3 * i + 1] >> 4 | ((uint16_t)a[3 * i + 2] & 0xff) << 4;
    }
}

extern void doublebasemul_asm(int16_t *r, const int16_t *a, const int16_t *b,
                              int32_t zeta);
/*************************************************
 * Name:        poly_frombytes_mul
 *
 * Description: Multiplication of a polynomial with a de-serialization of
 *another polynomial
 *
 * Arguments:   - poly *r:                pointer to output polynomial
 *              - const unsigned char *a: pointer to input byte array (of
 ...
 *into r
 **************************************************/
void poly_noise_eta1(poly *r, const unsigned char *seed, unsigned char nonce,
                     int add)
{
 ...
 *into r
 **************************************************/
void poly_noise_eta2(poly *r, const unsigned char *seed, unsigned char nonce,
                     int add)
{
 ...
 *multiplication. Cached values are generated during matrix-vector product.
...
extern void basemul_asm_opt_16_32(int32_t *, const int16_t *, const int16_t *,
                                  int16_t *);
void poly_basemul_opt_16_32(int32_t *r_tmp, const poly *a, const poly *b,
                            poly_half *a_prime)
{
 ...
 *              - int32_t *r_tmp:      array for accumulating unreduced results
 **************************************************/
extern void basemul_asm_acc_opt_32_32(int32_t *, const int16_t *,
                                      const int16_t *, int16_t *);
void poly_basemul_acc_opt_32_32(int32_t *r_tmp, const poly *a, const poly *b,
                                poly_half *a_prime)
{
 ...
 *              - int32_t *r_tmp:      array for accumulating unreduced results
 **************************************************/
extern void basemul_asm_acc_opt_32_16(int16_t *, const int16_t *,
                                      const int16_t *, int16_t *, int32_t *);
void poly_basemul_acc_opt_32_16(poly *r, const poly *a, const poly *b,
                                poly_half *a_prime, int32_t *r_tmp)
{
    basemul_asm_acc_opt_32_16(r->coeffs, a->coeffs, b->coeffs, a_prime->coeffs,
                              r_tmp);
}
