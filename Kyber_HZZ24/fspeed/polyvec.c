 ...
 *for KYBER_POLYVECCOMPRESSEDBYTES)
 ...
 *              - unsigned char *a: pointer to input byte array (of length
    ...
    for (i = 0; i < KYBER_K; i++) {
        for (j = 0; j < KYBER_N / 4; j++) {
            r->vec[i].coeffs[4 * j + 0] =
                (((a[5 * j + 0] | (((uint32_t)a[5 * j + 1] & 0x03) << 8)) *
                  KYBER_Q) +
                 512) >>
                10;
            r->vec[i].coeffs[4 * j + 1] =
                ((((a[5 * j + 1] >> 2) |
                   (((uint32_t)a[5 * j + 2] & 0x0f) << 6)) *
                  KYBER_Q) +
                 512) >>
                10;
            r->vec[i].coeffs[4 * j + 2] =
                ((((a[5 * j + 2] >> 4) |
                   (((uint32_t)a[5 * j + 3] & 0x3f) << 4)) *
                  KYBER_Q) +
                 512) >>
                10;
            r->vec[i].coeffs[4 * j + 3] =
                ((((a[5 * j + 3] >> 6) |
                   (((uint32_t)a[5 * j + 4] & 0xff) << 2)) *
                  KYBER_Q) +
                 512) >>
                10;
        }
 ...
 *              - const polyvec *a: pointer to input vector of polynomials (of
 *length KYBER_POLYVECBYTES)
 **************************************************/
