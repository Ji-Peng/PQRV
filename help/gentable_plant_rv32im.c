...
#define KYBER_Q 3329
#define QINV 0x6ba8f301  // Q^{-1} mod^{+-} 2^32
#define R2modQ 1976      // -2^32 mod^{+-} Q
...
void GenTables_7layer(void)
{
    ...
    r2modq = R2modQ;
    invN = INVN;
    memset(table_zetaqinv, 0, 128 * sizeof(uint32_t));
    for (j = 0; j < 127; j++) {
        expmod_int16(&t0, &root, treeNTTMerged_7layer[j], &q);
        mulmod_int16(&t0, &t0, &r2modq, &q);
        mul_int32(&table_zetaqinv[j], &t0, &qinv);
    }
    for (j = 0; j < 127; j++)
        printf("%d, ", table_zetaqinv[j]);
    printf("\n\n");
    ...
    mul_int32(&table_zetaqinv[127], &t1, &qinv);
